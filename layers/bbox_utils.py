...
def center_distance(box_a, box_b):
    ...
    A = box_a.size(0)
    B = box_b.size(0)
    a_x = ((box_a[:, 2] + box_a[:, 0])/2).unsqueeze(1).expand(A, B)
    a_y = ((box_a[:, 3] + box_a[:, 1])/2).unsqueeze(1).expand(A, B)
    b_x = ((box_b[:, 2] + box_b[:, 0])/2).unsqueeze(0).expand(A, B)
    b_y = ((box_b[:, 3] + box_b[:, 1])/2).unsqueeze(0).expand(A, B)
    distance = (a_x - b_x)*(a_x - b_x) + (a_y - b_y)*(a_y - b_y)
    return distance # [A,B]
